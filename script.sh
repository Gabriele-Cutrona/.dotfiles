...
echo "Keyd with my config? (caps lock = esc if pressed, ctrl if hold / alt + hjkl = arrows, ctrl + u/d = page up/down, ctrl+h/l = backspace/delete, alt/altgr = enter on tap) y/n"
read -r KEYD
if [[ $KEYD == "y" ]]; then
   sudo pacman -S keyd
